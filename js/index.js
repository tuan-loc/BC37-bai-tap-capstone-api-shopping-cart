...
const deleteProductInCart = (idProduct) => {
  let foundIndex = cart.findIndex((item) => item.product.id === idProduct);
  cart.splice(foundIndex, 1);
...
const order = () => {
  let billHTML = `
    <div class="invoice" style="height: 600px; width: 400px;">
      <div>
        <div class="order-details">
          <em>your order has been placed</em>
          <p>Your order-id is : <span>${
            Math.floor(Math.random() * (999 - 100)) + 100
          }</span></p>
          <p>your order will be delivered to you in 3-5 working days</p>
          <p>you can pay <span>$ ${calcTotalCash()}</span> by card or any online transaction method after the products have been dilivered to you</p>
        </div>
        <button onclick="okay(okay)" class="btn-ok">Okay</button>
        <button onclick="cancelInvoice()" class="btn-cancel-bill">Cancel</button>
      </div>
    </div>
  `;

  document.getElementsByClassName("order-now")[0].innerHTML = billHTML;
};
...
const okay = () => {
  ...
  let messageHTML = `
      <div class="invoice" style="height: 180px; width: 400px;">
        <div>
          <div class="order-details"><em class="thanks">Thanks for shopping with us</em></div>
          <button onclick="continueShopping()" class="btn-ok">continue</button>
        </div>
      </div>
    `;

  document.getElementsByClassName("order-now")[0].innerHTML = messageHTML;
  removeCart();
  renderCart(cart);
  calcTotalInCart();
};
...
const continueShopping = () => {
  document.getElementsByClassName("order-now")[0].innerHTML = "";
  document.getElementsByClassName("order-now")[0].style.width = "0";
};

const renderItemNames = (data) => {
  let namesHTML = "";
  for (let i in data) {
    namesHTML += `
      <span>${data[i].quantity} x ${data[i].product.name}</span>
    `;
  }
  return namesHTML;
};

const renderItemPrices = (data) => {
  let pricesHTML = "";
  for (let i in data) {
    pricesHTML += `
      <span>$ ${data[i].product.price}</span>
    `;
    if (i < data.length - 1) {
      pricesHTML += "+";
    }
